  ...
  public:
    ...
    OpenFile* Open(char *name) {
		...
		int openFile(char * fileName)
		{
			...
			if (freeSlot == -1) return -1; // if file table is full, return -1
			fileDescriptor = OpenForReadWrite(fileName, FALSE); // create sector
			if (fileDescriptor == -1) return -1; // can not create file, return -1
		...
		int Read(char* buffer, int numBytes, int index)
		{
			...
			if (open_f[index].open_file == NULL) return -1;
			int res = open_f[index].open_file->Read(buffer, numBytes);
		...
		int Write(char* buffer, int numBytes, int index) {
			if (index >= MAX_FILE) return -1;
			if (open_f[index].open_file == NULL) return -1;
			return open_f[index].open_file->Write(buffer, numBytes);
 	   }
		// BONUS
		int Seek(int position, int index)
		{
			if (index >= MAX_FILE || index <= 1) return -1;
			if (open_f[index].open_file == NULL) return -1;
			if (position == -1) position = open_f[index].open_file->Length();
			else if (position < 0 || position > open_f[index].open_file->Length()) return -1;
			return open_f[index].open_file->Seek(position);
		}
		...
		bool Remove(char *fileName) 
		{
			for (int i = 0; i < MAX_FILE; i++)
			{
				if (open_f[i].name == fileName) // if file is open, remove out of table and return		
				{
					delete open_f[i].open_file;
					open_f[i].open_file = NULL;
					delete open_f[i].name;
